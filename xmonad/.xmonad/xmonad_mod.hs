...
-- Layouts
myTitle = named "T" $ spacing 7 $ smartBorders $ ResizableTall 1 0.03 0.5 []
myMirr  = named "MT" $ spacing 7 $ smartBorders $ Mirror myTitle
...
myLayoutHook = id
	...
	$ allLayouts
	where
		allLayouts  = myTitle ||| myObig  ||| myMirr ||| myMosA ||| myTabM ||| myFull
...
myManageHook = (composeAll . concat $ [
		...
		, [className                       =? c --> doShift (myWorkspaces !! 5)      | c <- myGfxS]    --  Move myGfxS windows to workspace 5 by classname
		, [className                       =? c --> doShiftAndGo (myWorkspaces !! 6) | c <- myOtherS]  --  Move myOtherS windows to workspace 6 by classname and shift
	...
	where
		...
		myWebS          = ["Chromium", "google-chrome", "Google-chrome", "google-chrome-stable", "Google-chrome-stable"]
		myCodeS         = ["gvim", "Gvim", "GVIM"]
		myChatS         = ["irssi"]
...
myBottomStatusBar = "conky -c ~/.xmonad/.conkydzenbottom | dzen2 -x '0' -y '1200' -h '17' -w '1920' -fg '" ++ colorWhiteAlt ++ "' -bg '" ++ colorBlack ++ "' -fn '" ++ dzenFont ++ "' -ta r -e ''"
myTopStatusBar    = "conky -c ~/.xmonad/.conkydzentop | dzen2 -x '920' -y '0' -h '17' -w '1920' -fg '" ++ colorWhiteAlt ++ "' -bg '" ++ colorBlack ++ "' -fn '" ++ dzenFont ++ "' -ta r -e ''"
